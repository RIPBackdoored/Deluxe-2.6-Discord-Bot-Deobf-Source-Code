...
public class UnixCrypt {
    ...
    private static int[] body(int[] schedule, int eSwap0, int eSwap1) {
        ...
        do {
            if (j >= 25) {
                ...
                UnixCrypt.permOp(right, left, 1, 1431655765, results);
                right = results[0];
                left = results[1];
                UnixCrypt.permOp(left, right, 8, 16711935, results);
                left = results[0];
                right = results[1];
                UnixCrypt.permOp(right, left, 2, 858993459, results);
                right = results[0];
                left = results[1];
                UnixCrypt.permOp(left, right, 16, 65535, results);
                left = results[0];
                right = results[1];
                UnixCrypt.permOp(right, left, 4, 252645135, results);
                right = results[0];
                left = results[1];
    ...
    private static int[] desSetKey(byte[] key) {
        ...
        int[] results = new int[2];
        UnixCrypt.permOp(d, c, 4, 252645135, results);
        d = results[0];
        c = results[1];
        c = UnixCrypt.hPermOp(c, -2, -859045888);
        d = UnixCrypt.hPermOp(d, -2, -859045888);
        UnixCrypt.permOp(d, c, 1, 1431655765, results);
        d = results[0];
        c = results[1];
        UnixCrypt.permOp(c, d, 8, 16711935, results);
        c = results[0];
        d = results[1];
        UnixCrypt.permOp(d, c, 1, 1431655765, results);
        d = results[0];
        ...
        while (i < 16) {
            ...
            int s = SKB[0][(c &= 268435455) & 63] | SKB[1][c >>> 6 & 3 | c >>> 7 & 60] | SKB[2][c >>> 13 & 15 | c >>> 14 & 48] | SKB[3][c >>> 20 & 1 | c >>> 21 & 6 | c >>> 22 & 56];
            int t = SKB[4][(d &= 268435455) & 63] | SKB[5][d >>> 7 & 3 | d >>> 8 & 60] | SKB[6][d >>> 15 & 63] | SKB[7][d >>> 21 & 15 | d >>> 22 & 48];
