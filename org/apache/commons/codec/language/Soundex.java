...
implements StringEncoder {
    ...
    private char getMappingCode(String str, int index) {
        ...
        if (index <= 1) return mappedChar;
        if (mappedChar == '0') return mappedChar;
        char hwChar = str.charAt(index - 1);
        if ('H' != hwChar) {
            if ('W' != hwChar) return mappedChar;
        }
        if ((firstCode = this.map(preHWChar = str.charAt(index - 2))) == mappedChar) return '\u0000';
        if ('H' == preHWChar) return '\u0000';
        if ('W' != preHWChar) return mappedChar;
        return '\u0000';
    }
    ...
    private char map(char ch) {
        int index = ch - 65;
        if (index < 0) throw new IllegalArgumentException("The character is not mapped: " + ch);
        if (index < this.getSoundexMapping().length) return this.getSoundexMapping()[index];
